...
class Profile extends CI_Controller {
	...
	public function index() {
		...
		$grade_name = '';
		$bank_name = '';
		$query = $this->user->query_rec_single('id', $user_id, 'bz_employee');
		if(!empty($query)){
			foreach($query as $item){
				$gdept = $this->user->query_rec_single('id', $item->dept_id, 'bz_department');
				if(!empty($gdept)){
					foreach($gdept as $gd){
						$dept_name = $gd->name;
					}
				...
				$gbank = $this->user->query_rec_single('id', $item->bank_id, 'bz_bank');
				if(!empty($gbank)){
					foreach($gbank as $gb){
						$bank_name = $gb->name;
					}
				...
				$data['employ_date'] = $item->employ_date;
				$data['retire_date'] = $item->retire_date;
				$data['acc_no'] = $item->acc_no;
				$data['nk_name'] = $item->nk_name;
				$data['nk_address'] = $item->nk_address;
				$data['nk_phone'] = $item->nk_phone;
			}
		}
