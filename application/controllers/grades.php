...
class Grades extends CI_Controller {
	...
	public function index() {
		...
		if($get_id != ''){
			$gq = $this->user->query_rec_single('id', $get_id, 'bz_grade');
			foreach($gq as $item){
				$data['e_id'] = $item->id;
				$data['e_name'] = $item->name;	
				$data['e_salary'] = $item->salary;
			}
		}
		...
		if($del_id != ''){
			if($this->user->delete_rec('id', $del_id, 'bz_grade') > 0){
		...
		if($_POST){
			...
			if($dept_id != ''){
				...
				if($this->user->update_rec('id', $dept_id, 'bz_grade', $upd_data) > 0){
				...
				if($this->user->reg_rec('bz_grade', $reg_data) > 0){
					$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
				} else {
					$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
				}
		...
		//query uploads
		$data['allup'] = $this->user->query_rec('bz_grade');
